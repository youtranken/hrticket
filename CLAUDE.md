...
- **Capability chỉ feed `/me` (UX menu), chưa guard BE nào enforce.** SSA sửa ma trận runtime ở `modules/ssa` (cache 60s, bust khi ghi; locked-cells immutable). Default = mirror seed PRD §2 trong `capability-catalog.ts` (KHÔNG sửa seed.ts).
