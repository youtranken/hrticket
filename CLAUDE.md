...
- **Scheduler tick 60s** (đổi từ hourly): digest fire khi `vnParts(now).hour >= digest_hour` (tính giờ VN từ JS `now` injectable, KHÔNG SQL now() — test fake-clock truyền now); dedup `digest_log(recipient,date_vn)` cũng là cơ chế GỬI BÙ. Overdue/snooze flag trong scheduler tính bằng JS từ `now` (mirror SQL của 5.6) để fake-clock deterministic. Attachment-repair gate theo counter (~hourly) để khỏi quét mỗi phút.
